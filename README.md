...
| 二维码 | **31mm**（框 37mm） | **36.5mm**（框 44mm） | **37.4mm**（框 45mm） |
| 标题 / 提示字号 | 11pt / 6.5pt | 14pt / 8pt | 19pt / 9pt |
| 适合 | 后视镜旁、角落（省地方） | **想又大又清楚就选这个** | 挡风玻璃（横着贴更醒目） |
...
尺寸是**实测出来的上限**：贴纸内边距、标题高度、二维码高度加起来已经顶到贴纸可用高度
（50mm 款可用 46.5mm、内容 44.3mm；60mm 款可用 55.5mm、内容 53.2mm），再放大二维码就会把提示文字挤没。
三款渲染成图后都用 jsQR 实测解码通过（缩到 25% 仍能解出）。

> 50mm 见方塞不下「大二维码 + 看得清的字」——这是物理限制，不是没调好。
> 所以两款正方形都**去掉了「临时停靠 · 请多包涵」这行装饰字**，把高度全让给二维码；
> 既要码大又要字大，就用 **6×6** 那款。长方形够宽，仍保留这行副标题。

三款都只印「扫码挪车 / 二维码 / 车辆挡路请扫码联系车主」，
